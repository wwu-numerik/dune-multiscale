...
if [[ "x${TESTS}" != "xheadercheck" ]]; then
    ...
    ${SRC_DCTRL} ${BLD} --only=${MY_MODULE} bexec ninja -v -j 1 test_binaries
    ${SRC_DCTRL} ${BLD} --only=${MY_MODULE} bexec ninja check
    ...
    #${SUPERDIR}/scripts/bash/travis_upload_test_logs.bash ${SUPERDIR}/${MY_MODULE}/${DUNE_BUILD_DIR}/dune/multiscale/test/
