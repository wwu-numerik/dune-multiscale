...
void algorithm(std::string& macroGridName,
  ...
  DiscreteFunctionType oversampling_error_visualization(filename_ + " Visualization of the oversampling error",
                                                        discreteFunctionSpace_coarse);
  ...
  std::cout << "total_conservative_flux_jumps_[ loop_number_ ] = " << total_conservative_flux_jumps_[loop_number_]
            << std::endl;
  ...
  for (ElementLeafIterator it = gridView_coarse_new.begin< 0 >();
       it != gridView_coarse_new.end< 0 >(); ++it)
  {
    average_conservative_flux_jumps
      += loc_conservative_flux_jumps_[loop_number_][gridLeafIndexSet_coarse_new.index(*it)];
    cum += 1.0;
  }
       ...
       it != gridView_coarse_new.end< 0 >(); ++it)
  {
    ...
    RangeType loc_oversampling_indicator
      = loc_conservative_flux_jumps_[loop_number_][gridLeafIndexSet_coarse_new.index(*it)];
  ...
  DataOutputType oversampling_coarse_grid_dataoutput(
    gridPart_coarse.grid(), oversampling_coarse_grid_series, outputparam);
  // write data
