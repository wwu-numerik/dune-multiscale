...
void NumberPrinter::operator()( const blocked_range<size_t>& r) const {
...
void printNumbers (int n) {
    NumberPrinter p(n);
    parallel_for(blocked_range<size_t>(0,n), p, auto_partitioner());
}
...
int main(int argc, char** argv) {
  ...
  try {
    init(argc, argv);
    tbb::task_scheduler_init dd;

    //!TODO include base in config
    DSC_PROFILER.startTiming("total_cpu");

    const auto maxt = DSC_CONFIG_GET("threading.max_count", 240);
    DSC_LOG_ERROR << "max " << maxt;
    DSC_LOG_ERROR << "\ncur " << DS::threadManager().current_threads() << std::endl;
    printNumbers(1.3*maxt);
