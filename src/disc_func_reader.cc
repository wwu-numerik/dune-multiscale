...
#include <dune/multiscale/operators/errorestimation/elliptic/error_estimator.hh>
...
typedef OEMBICGSTABOp< DiscreteFunctionType, FEMMatrix > InverseFEMMatrix;
...
typedef ErrorEstimator< PeriodicDiscreteFunctionType, 
                        ...
                        DiffusionType, MassTermType, FirstSourceType > ErrorEstimatorType;
//! -----------------------------------------------------------------------------
  ...
  while( residual_L2_norm > tolerance )
   {
     ...
     InverseFEMMatrix fem_newton_biCGStab( fem_newton_matrix, 1e-7, 1e-8, 20000, VERBOSE );
...
int main(int argc, char** argv)
{
  ...
  int refinement_level_cellgrid = 0;

  // (starting) grid refinement level for solving the reference problem
  int refinement_level_referenceprob = info.getRefinementLevelReferenceProblem();
  ...
  int refinement_difference_for_referenceproblem = refinement_level_referenceprob - refinement_level_macrogrid_;
  ...
  fine_macro_grid_pointer->globalRefine( refinement_level_referenceprob );
#ifndef FINE_SCALE_REFERENCE
  refinement_level_referenceprob = 6;
#endif
  ...
  if (data_file.is_open())
            {
               ...
               data_file << "Delta (edge length of cell-cube) = " << delta_ << std :: endl << std :: endl << std :: endl;
            }
