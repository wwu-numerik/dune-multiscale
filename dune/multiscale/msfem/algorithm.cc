...
std::map<std::string, double> msfem_algorithm()
{
  ...
  if (problem.config().get("global.vtk_output", false)) {
    ...
    problem.getDiffusion().visualize(grid->leafGridView(),
    ...
    coarse_grid_visualization.visualize(OutputParameters(problem.config()).fullpath(coarse_grid_visualization.name()));
  }
