...
private:
  ...
  void getFluxes(const EntityType& coarse_entity, const DiscreteFunctionType& msfem_coarse_part,
    ...
    for (int i : {0, 1}) {
      ...
      DiscreteFunctionIO<MsFEMTraits::SubGridDiscreteFunctionType>::reader(cf_solution_location).read(0, conservative_flux_coarse_ent[i]);
    }
    ...
    for (IntersectionIteratorType face_it = coarseGridPart.ibegin(coarse_entity); face_it != endnit; ++face_it) {
      ...
      if (face_it->neighbor()) {
        ...
        for (int i : {0, 1}) {
          ...
          DiscreteFunctionIO<MsFEMTraits::SubGridDiscreteFunctionType>::reader(cf_solution_location_neighbor).read(0, conservative_flux_coarse_ent_neighbor[i]);
  ...
  void coarse_contribution(const DiscreteFunctionSpaceType& coarseDiscreteFunctionSpace,
    ...
    for (const auto& coarse_entity : coarseDiscreteFunctionSpace) {
      ...
      auto& discrete_function_reader = DiscreteFunctionIO<MsFEMTraits::SubGridDiscreteFunctionType>::reader(local_solution_location);
