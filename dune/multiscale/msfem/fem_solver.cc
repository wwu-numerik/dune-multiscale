...
#include <dune/gdt/operators/virtual_refined.hh>
...
auto make_solver_options(Solver& linear_solver) -> decltype(linear_solver.options()) {
...
void Elliptic_FEM_Solver::apply(CommonTraits::DiscreteFunctionType& solution) const {
  ...
  linear_solver.apply(rhs_vector, solution_vector, make_solver_options(linear_solver));
...
VirtualRefinedElliptic_FEM_Solver::VirtualRefinedElliptic_FEM_Solver(const Problem::ProblemContainer& problem,
                                                                     GridPtrType grid)
  : grid_(grid)
  , space_(CommonTraits::SpaceChooserType::PartViewType::create(*grid_, CommonTraits::st_gdt_grid_level))
  , solution_(space_, "fem_solution")
...
VirtualRefinedElliptic_FEM_Solver::VirtualRefinedElliptic_FEM_Solver(const DMP::ProblemContainer& problem)
  : VirtualRefinedElliptic_FEM_Solver(problem, make_fine_grid(problem, nullptr, false)) {}

VRfTraits::ConstDiscreteFunctionType& VirtualRefinedElliptic_FEM_Solver::solve() {
  apply(solution_);
  return solution_;
}

void VirtualRefinedElliptic_FEM_Solver::apply(VRfTraits::DiscreteFunctionType& solution) const {
  MS_LOG_DEBUG_0 << "Solving linear problem with standard FEM" << std::endl;
  ...
  auto& solution_vector = solution.vector();
  // left hand side (elliptic operator)
  VRfOperatorType elliptic_operator(problem_.getDiffusion(), system_matrix, space);
  // right hand side
  GDT::Functionals::L2Volume<Problem::SourceType, CommonTraits::GdtVectorType, VRfTraits::SpaceType> force_functional(
      problem_.getSource(), rhs_vector, space);
  GDT::Functionals::L2Face<Problem::NeumannDataBase, CommonTraits::GdtVectorType, VRfTraits::SpaceType>
      neumann_functional(neumann, rhs_vector, space);
  // dirichlet boundary values
  VRfTraits::DiscreteFunctionType dirichlet_projection(space);
  GDT::Operators::DirichletProjectionLocalizable<GridViewType, Problem::DirichletDataBase,
                                                 VRfTraits::DiscreteFunctionType>
      dirichlet_projection_operator(space.grid_view(), boundary_info, dirichlet, dirichlet_projection);
  DSC_PROFILER.startTiming("fem.assemble");
  // now assemble everything in one grid walk
  GDT::SystemAssembler<VRfTraits::SpaceType> system_assembler(space);
  system_assembler.add(elliptic_operator);
  //  system_assembler.add(force_functional);
  //  system_assembler.add(neumann_functional, new DSG::ApplyOn::NeumannIntersections<GridViewType>(boundary_info));
  //  system_assembler.add(dirichlet_projection_operator, new DSG::ApplyOn::BoundaryEntities<GridViewType>());
  ...
  // solve the system
  DSC_PROFILER.startTiming("fem.solve");
  const Stuff::LA::Solver<VRfTraits::LinearOperatorType, typename VRfTraits::SpaceType::CommunicatorType> linear_solver(
      system_matrix, space.communicator());
