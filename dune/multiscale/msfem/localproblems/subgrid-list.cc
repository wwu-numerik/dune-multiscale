    ...
    silent_(silent)
{
  ...
  // ! ----------- create subgrids --------------------
  const HostGridLeafIndexSet& coarseGridLeafIndexSet = coarseSpace.gridPart().grid().leafIndexSet();
  EnrichmentMatrixType        enriched(boost::extents[number_of_coarse_grid_entities][hostGrid.size(
                                                                                        0)][max_num_layers + 1]);
  ...
  for (const auto& host_entity : hostSpace_) {
    ...
    for (HostIntersectionIterator iit = hostGridPart.ibegin(host_entity); (iit != iend) && all_neighbors_have_same_father; ++iit) {
      if (iit->neighbor()) {
