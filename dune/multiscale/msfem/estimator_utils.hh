...
struct EstimatorUtils {
  ...
  static bool point_on_face(const typename EstimatorType::Intersection& face,
    ...
    if (corner_0[0] == corner_1[0]) {
      if (point[0] != corner_0[0]) {
        ...
        const typename EstimatorType::RangeType lambda = (point[1] - corner_1[1]) / (corner_0[1] - corner_1[1]);
        return ((lambda >= 0.0) && (lambda <= 1.0));
      }
    } else {
      const typename EstimatorType::RangeType lambda = (point[0] - corner_1[0]) / (corner_0[0] - corner_1[0]);

      if ((lambda >= 0.0) && (lambda <= 1.0)) {
        const typename EstimatorType::RangeType convex_comb = (lambda * corner_0[1]) + ((1.0 - lambda) * corner_1[1]);
  ...
  flux_contributions(const typename EstimatorType::SubGridDiscreteFunctionSpaceType& localDiscreteFunctionSpace,
                     ...
                     typename EstimatorType::FluxContainerType& cflux_neighbor_ent_host, const int index_coarse_entity,
    ...
    for (const auto& sub_entity : localDiscreteFunctionSpace) {
      ...
      auto father_of_sub_grid_entity =
          Stuff::Grid::make_father(coarseGridLeafIndexSet, host_entity_pointer, level_difference);
      ...
      for (auto face_it_U_T = fineDiscreteFunctionSpace.gridPart().ibegin(host_entity); face_it_U_T != end_it_U_T;
           ++face_it_U_T) {
        ...
        for (std::size_t quadraturePoint = 0; quadraturePoint < numQuadraturePoints; ++quadraturePoint) {
          ...
          RangeType jump_contribution =
              gradient_msfem_coarse_ent[0][0] * (/*fabs*/(value_ent_e0) + /*-fabs*/(value_neighbor_ent_e0));
          jump_contribution +=
              gradient_msfem_coarse_ent[0][1] * (/*fabs*/(value_ent_e1) + /*-fabs*/(value_neighbor_ent_e1));

          jump[relevant_face_index] += H_E * integrationFactor * quadratureWeight * pow(jump_contribution, 2.0);

          jump_contribution =
              gradient_msfem_coarse_neighbor_ent[0][0] * (/*fabs*/(value_ent_e0) + /*-fabs*/(value_neighbor_ent_e0));
          jump_contribution +=
              gradient_msfem_coarse_neighbor_ent[0][1] * (/*fabs*/(value_ent_e1) + /*-fabs*/(value_neighbor_ent_e1));
          ...
          cjump += fabs(value_neighbor_ent_e0 * coarse_jump_contribution[0][0] +
                        value_neighbor_ent_e1 * coarse_jump_contribution[0][1]);
