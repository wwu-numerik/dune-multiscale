...
public:
  ...
  void run() {
    ...
    std::vector< std::vector< unsigned int > > symgramcombs( dfList_.size(), std::vector< unsigned int >(
                                                               dfList_.size(), 0) );
    std::vector< unsigned int > veccombs(dfList_.size(), 0);
    #endif // ifdef GRIDWALK_TEST
    ...
    do {
      ...
      if (block1_.last() < dfList_.size() - 1)
      {
        ...
        do {
          // move second block of functions
...
private:
  ...
  } // computeBlockContributionsGramSym
  ...
  } // computeBlockContributionsVector
...
public:
  ...
  template< class DiscreteFunctionType >
  class OpEvals
    : public OpEvalsBase< DiscreteFunctionType >
  {
