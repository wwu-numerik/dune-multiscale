...
{ \
  template<typename TType> static TType makeT();  \
  template<typename Attribute> struct YesType  \
  { char small; }; \
  struct NoType \
  { char big[2]; }; \
  static NoType test(...); \
...
template<typename T> static T makeT();
template<typename T> static T& makeRT();
