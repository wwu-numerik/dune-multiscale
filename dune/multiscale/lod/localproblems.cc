...
LodLocalProblemDataOutputParameters::LodLocalProblemDataOutputParameters()
  : OutputParameters(DSC_CONFIG_GET("global.datadir", "data") + "/local_problems/") {}

std::unique_ptr<LodLocalProblemSolver::InverseLocProbLinearOperatorTypeType>
LodLocalProblemSolver::make_inverse_operator(LodLocalProblemSolver::LocProbLinearOperatorTypeType& problem_matrix) {
  const auto solver =
...
*
*/
void LodLocalProblemSolver::solveAllLocalProblems(const CoarseEntityType& coarseCell,
...
void LodLocalProblemSolver::solvelocalproblem(JacobianRangeType& e, SubDiscreteFunctionType& local_problem_solution,
...
void LodLocalProblemSolver::preprocess_corrector_problems(const int coarse_index,
...
void LodLocalProblemSolver::solve_corrector_problem_lod(
    JacobianRangeType& e,
    // the matrix in our linear system of equations
    // in the non-linear case, it is the matrix for each iteration step
...
void LodLocalProblemSolver::solve_dirichlet_corrector_problem_lod(
    // the matrix in our linear system of equations
    // in the non-linear case, it is the matrix for each iteration step
    LocProbLinearOperatorTypeType& locprob_system_matrix, MatrixType& lm_system_matrix,
    SubDiscreteFunctionType& local_corrector, const int coarse_index /*= -1*/) const {
...
void LodLocalProblemSolver::solve_neumann_corrector_problem_lod(
    // the matrix in our linear system of equations
    // in the non-linear case, it is the matrix for each iteration step
    LocProbLinearOperatorTypeType& locprob_system_matrix, MatrixType& lm_system_matrix,
    SubDiscreteFunctionType& local_corrector, const int coarse_index /*= -1*/) const {
...
void LodLocalProblemSolver::subgrid_to_hostrid_function(const SubDiscreteFunctionType& sub_func,
...
void LodLocalProblemSolver::output_local_solution(const int coarse_index, const int which,
  ...
  LodLocalProblemDataOutputParameters outputparam;
...
void LodLocalProblemSolver::output_local_solution(const int coarseIndex, const int which,
...
void LodLocalProblemSolver::assemble_all_clement_lod()
{
