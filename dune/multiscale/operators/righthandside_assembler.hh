	...
	for( int i = 0; i < numDofs; ++i ) //Laufe ueber alle Knoten des entity-elements auf dem wir uns befinden
	    ...
	    baseSet.jacobian( i, quadrature[quadraturePoint], grad_phi_x );
            ...
            elementOfRHS[ i ] += det * quadrature.weight( quadraturePoint ) * (f_x * phi_x);
      	    ...
      	    elementOfRHS[ i ] -= det * quadrature.weight( quadraturePoint ) * (diffusive_flux_in_grad_old_u_H[0] * grad_phi_x[0]);
...
//  where we already added \int_T f \phi in the previous step )
   ...
   ///############################ The rhs-assemble()-methods for non-linear elliptic problems, solved with the heterogenous multiscale method  #########################################
    ...
    template< int polOrd, class FirstSourceType, class DiffusionOperatorType, class PeriodicDiscreteFunctionType, class CellProblemNumberingManagerType >
    void assemble_for_HMM_Newton_method( const FirstSourceType &f,
                                               DiffusionOperatorType &A,
                                         const DiscreteFunctionType &old_u_H, //old_u_H from the last iteration step
                                         // to obtain some information about the periodic discrete function space (space for the cell problems)
                                         ...
                                         std :: string filename = "no_file" )
    {
      ...
      if ( filename == "no_file" )
       {
         std :: cout << "ERROR! No 'filename' in RHSAssembler method 'assemble_for_HMM_Newton_method', but no AD_HOC_COMPUTATION initialized. Therefore the location of the saved cell problems is not available. Please define AD_HOC_COMPUTATION (ad hoc computation of the cell problems) or pass a corresponding 'filename'-variable!" << std :: endl;
         std :: abort();
       }
      ...
      for( IteratorType macro_grid_it = discreteFunctionSpace.begin(); macro_grid_it != macro_grid_endit; ++macro_grid_it )
      {
        ...
        // the fine scale reconstructions are only available for the barycenter of the macro grid entity (=> only available for the canonical one point quadrature on this element)
	...
	for( int i = 0; i < numDofs; ++i )
          {
            ...
            //--------------- the contribution of the jacobian of the diffusion operator, evaluated in the old reconstructed macro solution -------------------------------
            ...
            const double macro_entity_volume = one_point_macro_quadrature.weight( 0 /*=quadraturePoint*/ ) * macro_grid_geometry.integrationElement( local_macro_point );
	    ...
	    macro_grid_baseSet.jacobian( i, one_point_macro_quadrature[ 0 ], grad_Phi_x );
            ...
            discrete_function_reader_discFunc.read( number_of_entity, corrector_old_u_H );
             #ifdef TFR
             discrete_function_reader_baseSet.read( cp_num_manager.get_number_of_cell_problem( macro_grid_it, i ), corrector_Phi_i );
             #endif
            #endif
            ...
            for( IteratorType micro_grid_it = periodicDiscreteFunctionSpace.begin(); micro_grid_it != micro_grid_end; ++micro_grid_it )
              {
                ...
                typename PeriodicDiscreteFunctionType::LocalFunctionType loc_corrector_old_u_H = corrector_old_u_H.localFunction( micro_grid_entity );

                #ifdef TFR
                typename PeriodicDiscreteFunctionType::LocalFunctionType loc_corrector_Phi_i = corrector_Phi_i.localFunction( micro_grid_entity );
                #endif
                  ...
                  for( size_t microQuadraturePoint = 0; microQuadraturePoint < numQuadraturePoints; ++microQuadraturePoint )
                    {
                      ...
                      fine_scale_contribution += cutting_function * weight_micro_quadrature * ( diffusive_flux[ 0 ] * grad_reconstruction_Phi_i[ 0 ]);
                      #else
