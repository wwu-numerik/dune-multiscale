...
void ExactSolution::evaluate(const ExactSolution::DomainType &x, const ExactSolution::TimeType &, ExactSolution::RangeType &y) const {
  evaluate(x, y);
}

void ExactSolution::evaluateJacobian(const ExactSolution::DomainType &, ExactSolution::JacobianRangeType &) const {
  DUNE_THROW(Dune::NotImplemented, "Dummy body for all-problem compile");
}

void Diffusion::jacobianDiffusiveFlux(const Diffusion::DomainType &x, const Diffusion::JacobianRangeType &position_gradient, const Diffusion::JacobianRangeType &direction_gradient, Diffusion::JacobianRangeType &flux) const {
...
void Diffusion::diffusiveFlux(const Diffusion::DomainType &x, const Diffusion::JacobianRangeType &gradient, Diffusion::JacobianRangeType &flux) const {
  double coefficient = 2.0 + sin(2.0 * M_PI * (x[0] + x[1]) / constants().epsilon);
...
void FirstSource::evaluate(const FirstSource::DomainType &x, const FirstSource::TimeType &, FirstSource::RangeType &y) const {
  evaluate(x, y);
}
