...
// No heterogenity in the advective part so that we can use that b is divergence-free and with zero meanvalue. This means that we can make a very fine scale
...
// G(t,x) = second source term with the structure G = G(t,x) (=> no micro-scale dependency). Note that 'G' is directly implemented! ( we do not implement '- div G'!)
...
namespace Dune
{
  ...
  //Note: 'Function' is only a class-template, 'Function< FunctionSpaceImp, FirstSource< FunctionSpaceImp > >' is a class!

  //template parameters of 'Function': - 'FunctionSpaceImp' the function space where the Function ('FirstSource') shall be in, and - 'FirstSource': see Barton-Nackman trick.
...
//The value of the right hand side (i.e. the value of 'f') at 'x' is accessed by the method 'evaluate'. That means 'y := f(x)' and 'y' is returned. It is only important that 'RHSFunction' knows the function space ('FuncSpace') that it is part from. (f \in FunctionSpace)
 ...
 
 {
  ...
  public:
    ...
    // theta is a parameter that is only required for the cell problems. In our case it is completly ignored by defaulting to 'theta = 1'
    const double theta_;
    const TimeType t_;
  ...
  //! The next class could be used to define the exact solution of the problem above. But since it very problematic to find this exact solution, the following problem is only a dummy. To compare (later on) the numerical solution with the exact solution in order to calculate the EOC (Estimated Order of Convergence), we will need to homogenize the problem to get a numerical approximation of the exact solution.
...
//Two different kinds of evaluate-methods will be given. One to evaluate A and one to evaluate A^{\epsilon}=A(\frac{\cdot}{\epsilon}).
  template< class FunctionSpaceImp >
  class Advection
  ...
  public:
    ...
    inline void getAverage ( DomainType &bar_b ) const
    {
      ...
      // for generalization change for instance (in the classes DriftedDiffusion and DriftedAdvection) "bar_b * t" to some "B(t)"!
    ...
    { 
      ...
      if(i == 0)
        {z = 1.5 * sin( 2.0 * M_PI * ( y[0] / epsilon ) ) * sin( 2.0 * M_PI * ( y[1] / epsilon ) );} 
      else 
...
template< class FunctionSpaceImp >
  ...
  public:
    ...
    // the tensor will be called with time t, since it is newly created for any time step, t is just a dummy (the entries change automaticaly)
    inline void evaluate ( const int i,
                           const int j,
                           const DomainType &x,
                           const TimeType &t,
  ...
  public:
    ...
    inline explicit DefaultDummyFunction ( const FunctionSpaceType &functionSpace, const double &epsilon, const double &time_step_size)
    : BaseType( functionSpace ),
      functionSpace_( &functionSpace ),
      epsilon_( epsilon ),
      time_step_size_ ( time_step_size )
