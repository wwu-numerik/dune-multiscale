...
// {abort();}
    ...
    for (IteratorType fine_it = fine_discreteFunctionSpace.begin(); fine_it != fine_end; ++fine_it)
    {
      // wir klappern jetzt alle Makroelemente ab, um das zum fine-grid-element gehörende, relevante
      // coarse-grid-element zu finden
      EntityPointerType relevant_coarse_entity = coarse_discreteFunctionSpace.begin();  // still a dummy!
      bool relevant_coarse_entity_found = false;

      for (int level_difference = minimum_level_difference;
           level_difference <= maximum_level_difference;
           ++level_difference)
      {
        EntityPointerType fine_father_entity = fine_it;
        for (int lev = 0; lev < level_difference; ++lev)
        ...
        if ( (relevant_coarse_entity_found == false) && (level_difference == maximum_level_difference)
             && (error_in_compuation == false) )
        {
          std::cout << "In class >>ImprovedL2Error<<, in method >>norm_adaptive_grids<< :" << std::endl;
          std::cout
          << "No corresponding coarse grid entity found for fine grid entity => Error in computation of L2 error"
      ...
      // wir verwenden die Eckpunkte des Referenzelements ( (0,0), (1,0) und (0,1) ) und mappen die gloabl in der
      // aktuell coarse-grid entity.
...
// ! Old alternative - Schneller, aber funktioniert noch nicht richtig, da die Nummerierung der entities nicht ganz so
// ist, wie erwartet:
  #if 0
  // for the following method, the DiscreteFunction 'fine_disc_func' needs to be defined on a gridPart that is a
  // refinment of the gridPart of 'coarse_disc_func'
  ...
  RangeFieldType norm2(const DiscreteFunctionType& coarse_disc_func,
    ...
    for (IteratorType coarse_it = coarse_discreteFunctionSpace.begin(); coarse_it != coarse_entity_end; ++coarse_it)
    {
      ...
      { new_pol_ord = (reference_fine_entity_level - coarse_entity_level + 1) * quadOrd; } else
...
// Problem ist, dass nicht alle Fine-grid elemente besucht werden
            ...
            for (int coarse_qp = 0; coarse_qp < coarse_quadNop; ++coarse_qp)
            {
              RangeType actual_distance
                = sqrt( pow(coarse_quad_point[coarse_qp][0] - fine_quad_point[0],
                            2.0) + pow(coarse_quad_point[coarse_qp][1] - fine_quad_point[1], 2.0) );
