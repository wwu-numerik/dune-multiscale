...
Dune::Multiscale::DiscreteFunctionIO::get_disk(std::string filename) {
  return *get(disk_, filename, filename);
}

Dune::Multiscale::MemoryBackend &
Dune::Multiscale::DiscreteFunctionIO::get_memory(
    std::string filename,
    Dune::Multiscale::IOTraits::GridViewType &grid_view) {
  return *get(memory_, filename, grid_view, filename);
}

Dune::Multiscale::MemoryBackend &Dune::Multiscale::DiscreteFunctionIO::memory(
    std::string filename,
    Dune::Multiscale::IOTraits::GridViewType &grid_view) {
...
Dune::Multiscale::DiskBackend &
Dune::Multiscale::DiscreteFunctionIO::disk(std::string filename) {
...
void Dune::Multiscale::DiscreteFunctionIO::clear() {
  ...
  DSC_LOG_DEBUG << (boost::format("cleared %d in-memory functions\ncleared %d "
                                  "on-disk   functions\nfor %s\n") %
                    th.memory_.size() % th.disk_.size() %
                    DSC::getTypename(th)).str();
  th.memory_.clear();
  th.disk_.clear();
}
