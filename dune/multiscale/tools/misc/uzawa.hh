...
private:
  ...
  */
  ...
  // dune_static_assert( (Conversion< ADomainType, ARangeType >::sameType), "DomainType must equal RangeType for
  // Operator A" );
  // dune_static_assert( (Conversion< ADomainType, BDomainType >::sameType), "B has to map from A DomainType to B
  // RangeType" );
...
public:
   ...
   */
  UzawaSolver(ARangeFieldType epsilon, unsigned int maxIterations, bool verbose)
    : epsilon_(epsilon)
   ...
   *  \param[in]  maxIterations  maximum number of CG iterations
   */
  UzawaSolver(ARangeFieldType epsilon, unsigned int maxIterations)
    : epsilon_(epsilon)
    , maxIterations_(maxIterations)
...
private:
  // prohibit copying
  UzawaSolver(const UzawaSolver&) = delete;
...
public:
  ...
  void solve(const AOperatorType& inverseLaplace, const BOperatorType& B, const ARangeType& b, const BRangeType& g,
    ...
    for (unsigned int count = 0; (residuum > tolerance) && (count < maxIterations_); ++count) {
      ...
      if (verbose) {
        std::cerr << "Uzawa-Iteration: " << count << ", Residuum: " << residuum << std::endl;
  ...
  int iterations() const { return realCount_; }
...
struct UzawaInverseOp : public Operator<typename DFV::RangeFieldType, typename DFV::RangeFieldType, DFV, DFV> {
private:
...
public:
   ...
   */
  UzawaInverseOp(const AType& A, const BType& B, double /*redEps*/, double absLimit, int maxIter, bool verbose)
    : A_(A)
    , B_(B)
   ...
   *  \param[in] maxIter maximal iteration steps
   */
  UzawaInverseOp(const AType& A, const BType& B, double /*redEps*/, double absLimit,
  ...
  */
  virtual void operator()(const DiscreteVelocityFunctionType& arg1, const DiscretePreasureFunctionType& arg2,
                          DiscreteVelocityFunctionType& dest1, DiscretePreasureFunctionType& dest2) const {
    solver_.solve(A_, B_, arg1, arg2, dest1, dest2);
  }

  virtual void operator()(const DiscreteVelocityFunctionType& /*arg1*/, DiscreteVelocityFunctionType& /*dest1*/) const {
  ...
  //! return average communication time during last solve
  double averageCommTime() const { return solver_.averageCommTime(); }
