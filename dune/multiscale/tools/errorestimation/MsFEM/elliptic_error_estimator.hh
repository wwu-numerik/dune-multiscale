...
//! old version
                ...
                LocalGridEntityQuadratureType local_grid_quadrature( local_grid_entity, 2*localDiscreteFunctionSpace.order()+2 );
                ...
                for( size_t localQuadraturePoint = 0; localQuadraturePoint < numQuadraturePoints; ++localQuadraturePoint )
                 {
                    ...
                    const typename LocalGridEntityQuadratureType::CoordinateType &local_subgrid_point = local_grid_quadrature.point( localQuadraturePoint );
                    ...
                    SubGridLocalFunctionType localized_local_problem_solution_e0 = local_problem_solution_e0.localFunction( local_grid_entity );
                    SubGridLocalFunctionType localized_local_problem_solution_e1 = local_problem_solution_e1.localFunction( local_grid_entity );
                    ...
                    for( int k = 0; k < dimension; ++k )
                       projection_error_gradient[ 0 ][ k ] = grad_msfem_fine_part[ 0 ][ k ] 
                          - ( grad_loc_sol_e0[ 0 ][ k ] * grad_msfem_coarse_part[ 0 ][ 0 ] + grad_loc_sol_e1[ 0 ][ k ] * grad_msfem_coarse_part[ 0 ][ 1 ] );
...
value = 0.0;
value += pow ( loc_msfem_fine_part - ( loc_sol_e0 * grad_msfem_coarse_part[ 0 ][ 0 ] + loc_sol_e1 * grad_msfem_coarse_part[ 0 ][ 1 ] ) , 2.0 );
#endif   
