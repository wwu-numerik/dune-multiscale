...
namespace Dune
{
  ...
  public:
    ...
    template< class MatrixType >
    void assemble_jacobian_matrix ( const DomainType &x_T, JacobianRangeType &grad_coarse_function, DiscreteFunction &old_fine_function, MatrixType &global_matrix ) const;
    ...
    void assemble_jacobian_corrector_cell_prob_RHS
  ...
  // here we obtain the contribution of the jacobian matrix of the diffusion operator evaluated in the gradient of a certain discrete function (in case of the Newton method, it is the preceeding iterate v_h^{(n-1)} )
 ...
 // (cell problem for the macro grid element 'T')
  ...
  template< class PeriodicDiscreteFunctionImp, class DiffusionImp >
  template< class MatrixType >
  void DiscreteCellProblemOperator< PeriodicDiscreteFunctionImp, DiffusionImp >::assemble_jacobian_matrix ( const DomainType &x_T, // macroscopic quadrature point
                                JacobianRangeType &grad_coarse_function, // the gradient of the macroscopic function (that we want to reconstruct) evaluated in x_T
                                PeriodicDiscreteFunctionImp &old_fine_function, // the microscopic function (fine-scale correction) from the last iteration step of the Newton method
    ...
    for( Iterator it = periodicDiscreteFunctionSpace_.begin(); it != end; ++it )
    {
      ...
      for( size_t quadraturePoint = 0; quadraturePoint < numQuadraturePoints; ++quadraturePoint )
      {
        ...
        for( unsigned int i = 0; i < numBaseFunctions; ++i )
        {
          ...
          // jacobian of diffusion operator evaluated in (x,grad coarse + grad fine) in direction of the gradient of the current base function
  ...
  // (in this case a Newton method is required, which is why there is an additional dependency on an old fine-scale discrete function v_h^{(n-1)} / old iteration step )
  ...
  void DiscreteCellProblemOperator< DiscreteFunctionImp, DiffusionImp >::assembleCellRHS_nonlinear
       ( // the global quadrature point in the macro grid element T
         const DomainType &x_T,
         //in the linear setting we typically reconstruct macroscopic base functions, in the non-linear setting we are in a more general setting.
...
// end nonlinear case.
            ...
            PeriodicDiscreteFunctionType &jac_cor_cell_problem_solution)
    {
      ...
      CellFEMMatrix jac_cor_cell_system_matrix( "Jacobian Corrector Cell Problem System Matrix", periodicDiscreteFunctionSpace_, periodicDiscreteFunctionSpace_ );

      //! define the discrete (elliptic) cell problem operator
      // ( effect of the discretized differential operator on a certain discrete function )
      CellProblemOperatorType cell_problem_op( periodicDiscreteFunctionSpace_, diffusion_);
      // we are looking for the derivative of the operator in \nabla_x u_H^{(n-1)})(x_T) + \nabla_y Q_h( u_H^{(n-1)})(y) and in direction of the macroscopic base function
      ...
      PeriodicDiscreteFunctionType jac_cor_cell_problem_rhs( "rhs of jacobian corrector cell problem", periodicDiscreteFunctionSpace_ );
        ...
        for(IteratorType it = discreteFunctionSpace.begin(); it != endit ; ++it)
          {
           ...
           PeriodicDiscreteFunctionType cell_solution_on_entity( "corrector of macro discrete function" , periodicDiscreteFunctionSpace_ );
...
//!-------------------------------------
      ...
      if (data_file_)
      {
         if (data_file_->is_open())
         {
           ...
           (*data_file_) << "Minimum time for solving a cell problem = " << minimum_time_c_p << "s." << std :: endl;
           (*data_file_) << "Maximum time for solving a cell problem = " << maximum_time_c_p << "s." << std :: endl;
           (*data_file_) << "Average time for solving a cell problem = " << ((clock()-starting_time)/CLOCKS_PER_SEC)/discreteFunctionSpace.grid().size(0) << "s." << std :: endl;
           (*data_file_) << "Total time for computing and saving the cell problems = " << ((clock()-starting_time)/CLOCKS_PER_SEC) << "s," << std :: endl << std :: endl;
         }
      }
    ...
    // compute and save solutions of the jacobian corrector cell problems for the base function set of the 'discreteFunctionSpace' and for a fixed macroscopic discrete function
    // requires cell problem numbering manager
    template < class DiscreteFunctionImp, class CellProblemNumberingManagerImp >
        ...
        for(IteratorType it = discreteFunctionSpace.begin(); it != endit ; ++it)
          {
           ...
           PeriodicDiscreteFunctionType corrector_macro_discrete_function( "corrector of macro discrete function" , periodicDiscreteFunctionSpace_ );
           if (reader_is_open)
