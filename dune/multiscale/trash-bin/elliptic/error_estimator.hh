...
// \int_E h³_entity ([( A_h(x_j,y) (\nabla_x u_H(x_j) + \nabla_y (K_h(u_H))]_E * n)²
  RangeType errorXi
    ...
    // ! WICHTIGE NOTIZ: das hier ist natürlich alles suboptimal, da wir statt dem grid auf dem periodic disrecte
    // function space das grid auf dem normalen discrete function space verwenden. leider lässt sich das momentan noch
    // nicht umgehen. Damit ergeben sich zwei Problem:
...
// das folgende nutzt merfach direkt, dass bei LagrangeSpace mit PolOrder 1, die Gradienten auf jeder Entity konstant
// sind und es deshalb egal ist, welcher Quadraturpunkt verwendet wird. Wir verwenden für EntityQuadraturen stets die
// Mittelpunkt Quadratur. Damit ist der einzige Quadraturpunkt stets der Mittelpunkt der Entity: entityQuadrature, 0 (0
// =
// quadPoint).
...
// for(int entityQuadPoint = 0; entityQuadPoint < quadratureNop; ++entityQuadPoint)
      ...
      for (IntersectionIteratorType nit = gridPart.ibegin(entity); nit != endnit; ++nit)
      {
        // the following distinction is redundant if we are able to use the intersection iterator and face quadratures
        // for the periodic gridpart:
        ...
        if ( nit.neighbor() )    // if there is a neighbor entity
        {
          ...
          // innerFaceQuadrature and outerFaceQuadrature are equivalent, therefor we it is sufficient to stay restricted
          // to the innerFaceQuadrature
          ...
          // (if nit is a boundary face:)
          // ! hier verschenken wir nun etwas, solange wir den IntersectionIterator für nicht periodisches Gitter
          // verwenden. Hier verlieren wir die 'Randsprünge', da wir 'zu früh' die L²-Norm bilden
  ...
  RangeType errorMu(const EntityType& entity,
    ...
    // for further considerations with matrices of the type A(x,y) (whereas the dependency on the first variable x is a
    // 'real' dependency!) it may be better to use polynomial order = 2 instead of polynomial order = 0!
    ...
    // start loop over the quadrature points of the macro mesh element T_j
    {
      ...
      // start loop over the elements of the unit cell mesh
      {
        ...
        // This is important to compare A and A_h properly. A_h is defined by the values of A in the quadrature points
        // of the trivial quadrature: A_h(x,y)_{|T_j /times S_k^*} := A(x_j, y_k). Now if we used the same quadrature
        // for
        // A, then the error would be zero! (A_h(x_j, y_k) - A(x_j, y_k) = 0) This is wrong since the error should have
        // the same order as theory suggests. (generaly first order)
        ...
        for (int localQuadPoint = 0; localQuadPoint < localQuadratureNop; ++localQuadPoint)
        {
          ...
          if (homogeneous_structure == true)
          {
            ...
            for (int k = 0; k < dimension; ++k)
              y_eps_new[k] = globalGeometry.global( globalQuadrature.point(quadPointIn_T_j) )[k]
                             + epsilon_est_* localGeometry.global( localQuadrature.point(localQuadPoint) )[k];
        ...
        // averaging prevents that the maximum of all distances (A - A_h) is multiplied with the maximum of the weights,
        // which could enlarge the error and decrease the rate of convergence fundamentaly!
    ...
    for (IntersectionIteratorType nit = gridPart.ibegin(entity); nit != endnit; ++nit)
    {
      if ( nit.neighbor() )     // if there is a neighbor entity
      {
        ...
        const RangeType neighEntityVolume = neighborEntityQuadrature.weight(0)
                                            * globalNeighEntityGeometry.integrationElement(
          neighborEntityQuadrature.point(0) );
...
// ! End computing outer normal
        ...
        // Now gradientU_H_onNeighEntity and correctorU_H_onNeighEntity are determined, the corresponding base functions
        // on neighborEntity are no more important now.
...
// y_k is the current (local) quadrature point
          ...
          if (homogeneous_structure == true)
          {
            for (int k = 0; k < dimension; ++k)
              ...
              {
                tensor_.evaluate(l, k,
                                 globalNeighEntityGeometry.global( neighborEntityQuadrature.point(0) ),         // global
                                                                                                                // quad
                                                                                                                // point
                                 localGeometry.global( localQuadrature.point(0) ),            // local quad point
                                 a_neighEntity[l][k]);
            ...
            for (int k = 0; k < dimension; ++k)
              y_eps_neigh_entity[k] = globalNeighEntityGeometry.global( neighborEntityQuadrature.point(0) )[k]
                                      + epsilon_est_* localGeometry.global( localQuadrature.point(0) )[k];
