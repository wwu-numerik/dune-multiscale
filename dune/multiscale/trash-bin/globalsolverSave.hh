...
public:
  ...
  /*  The following typedefs are only important if the MatrixAssembler is a HM-FEM Assembler. In case of standard  *
     * FEM Assemblers, the following typedefs are simply dummies! */
...
private:
  typedef EllipticFEOp< DiscreteFunctionType, PeriodicDiscreteFunctionImp, TensorType, AdvectionType, MassTermType,
                        MatrixAssemblerImp > ThisType;
  typedef FEOp< DiscreteFunctionType, MatrixType,
                ThisType > BaseType;
...
public:
  ...
  // Corresponding matrix to only one element of the grid
...
public:
  ...
  template< class IrgendwasImp > RightHandSideAssembler(IrgendwasImp& h)
    : tensor_(NULL)
      , G_(NULL)
  {}

  // ! constructor - Tensor (A(x)), MassTerm (a(x)) and (-\div G)-Term
  template< class TensorType, class SecondSourceType > RightHandSideAssembler(const TensorType& tensor,
...
public:
  ...
  void assemble(const FunctionType& function,
    ...
    for (IteratorType it = discreteFunctionSpace.begin(); it != endit; ++it)
    {
      ...
      LocalFunctionType elementOfRHS = discreteFunction.localFunction(*it);     // *it zeigt auf ein bestimmtes Element
                                                                                // der entity
...
#endif // ifndef DUNE_GLOBALSOLVER_HH
