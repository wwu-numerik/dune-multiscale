  ...
  if (DSC_CONFIG_GET("problem.linear", true))
  {
    ...
    while (relative_newton_error_finescale > tolerance)
    {
      ...
      if (rhs_L2_norm < 1e-8)
      {
        ...
        DSC_LOG_INFO << "(L^2-Norm of current right hand side = " << rhs_L2_norm << " < 1e-8)" << std::endl;
        break;
      }
      // set Dirichlet Boundary to zero
      boundaryTreatment(system_rhs);

      const typename FEMTraits::InverseFEMMatrix fem_newton_biCGStab(system_matrix, 1e-7, 1e-7, 20000, true);
